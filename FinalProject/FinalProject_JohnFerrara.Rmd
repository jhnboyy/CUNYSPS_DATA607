...
As most know, in recent years the cost of housing has been outpacing wage growth^1^. Rising housing costs have led to a series of policy issues. While some impacts of high housing costs are obvious, others are less apparent. This analysis explores whether one such issue — the recovery of used syringes in public parks — can be linked to the housing crisis. In short, the overarching question: Does housing impact this seemingly separate public health and safety issue of public, or improper, syringe disposal?
...
- _Question 1_: Does the Number of Housing Related Construction Projects for the Previous 5 Years influence the Size of Adult Homeless Population?
    - *Null Hypothesis*: The number of housing related construction projects for the preceeding 5 year window period **does not** have an impact on the adult homeless population.
    - *Alternative Hypothesis*: The number of housing related construction projects for the preceeding 5 year window period **does** impact the adult homeless population.
    - *Independent Variable*: The independent variable in this question is the completed housing-related contstruction projects completed in the preceeding 5 year window from when the homesless population was counted. 
    - *Dependent Variable*: The dependent variable in this question is the Adult homeless population, as the assumption is as more housing construction projects are completed, more units will be available. Thus reducing unhoused individuals.

- _Question 2_: Does the size of the Adult Homeless Population influence the Number of Used Syringes recovered by NYC Parks?
    - *Null Hypothesis*: The size of the Adult homeless population **does not** influence the number of used syringes recovered in NYC Parks.
    - *Alternative Hypothesis*: The size of the Adult homeless population **does** the number of used syringes recovered in NYC Parks.
...
It should be noted that these are large questions. Questions that are much more nuanced and complex than any single analysis can outline, so while this study seeks to demonstrate a link between varying dimensions of public policy, it is by no means an attempt at a solution. Rather, the study seeks to determine if there is an overlap between these issues using several sources of public data.
...
## Year Coverage
#Syringes
unique(data_final$year[!is.na(data_final$total_syringe_ests)])#Syringe Counts have Values from 2017 through 2024

#Homeless Shelter Pops
unique(data_final$year[!is.na(data_final$avg_homeless_count)])# 2018 through 2024

#Construction Housing Projecrs ( this should match the Syringe Count b/c of processing)
unique(data_final$year[!is.na(data_final$housingsum)])# 2017 through 2024
```
...
### Histogram for Ditributions
``` {r hist_dist}
### Syringes
ggplot(data_final, aes(x = total_syringe_ests)) + geom_histogram(binwidth = 2000)
#### Floor at Zero, cant have negative Syringes. Log Transform Needed.
ggplot(data_final, aes(x = log(total_syringe_ests+1))) + geom_histogram(binwidth = 1)


### Avg. Annual Adult Homeless POpulation
ggplot(data_final, aes(x = avg_homeless_count)) + geom_histogram(binwidth = 200)
### Floor at Zero b/c cant have negative people. Log Transform needed.
ggplot(data_final, aes(x = log(avg_homeless_count+1))) + geom_histogram(binwidth = 1)


### Prev.5 Yr Window Completed Housing Construction Porject Sum
ggplot(data_final, aes(x = housingsum)) + geom_histogram(binwidth = 200)
## Has semi-hard Floor at zero, Trying Log Transform.
ggplot(data_final, aes(x = log(housingsum+1))) + geom_histogram(binwidth = 1)
## Still high count at 0, best I can do. 


### Prev.5 Yr Window Avg. YOY  Pct. Change Completed Housing Construction Projects
ggplot(data_final, aes(x = prev5_avg_yoy_change)) + geom_histogram(binwidth = 500)
#Checking Log Transform Distribution
ggplot(data_final, aes(x = log(prev5_avg_yoy_change+1))) + geom_histogram(binwidth = 1)
## Log looks much better for normal dist.

```
After taking a look at the histograms for each of these variables that I want to analyze, because of the zero floors for several of them and the original distributions, Log transformations provide a more normal distribution to analyze. Will use the log transformations in regressions. 
...
**MODEL 1: Relationship Between Logged Adult Homeless Population and Logged Completed Housing Construction Projects for Preceeding 5 Years at the Community District Level**
...
## Seems to be slight correlation in lower portion. However, line seems to have Zero Slope.
```
...
Model 1 is Valid. The slope of the line for Residuals and Predicated vaues is zero. Best fit. The residualts ar also normally distributed.

**MODEL 2: Relationship Between Logged Adult Homeless Population and Logged Average YOY Percent Change in Completed Housing Related Construction Projects for Preceeding 5 Years for Preceeding 5 Years at the Community District Level**
...
```
Model 2 is **NOT** Statisitcally Significant. Moving On.
...
Model 3 **is** Valid. Residual distribution is normal, and predicted values vs. residuals has slope if zero.
...
**MODEL 4: Relationship Between Logged Adult Homeless Population and Logged Total Number of Completed Housing Related Construction Projects for Preceeding 5 Years at the Borough Level**
...
MODEL 4 is valid. Redsiduals and Predicted values have slope of zero. The residuals have mostly normal distrubtion. Slight deviation, but normal.

**MODEL 5: Relationship Between Logged Adult Homeless Population and Logged Average YOY Percent Change in Completed Housing Related Construction Projects for Preceeding 5 Years at the Borough Level**
...
``` {r m5_check2}
# Residual Dist.
ggplot(data = m5, aes(x = .resid)) +geom_histogram(binwidth = 1) + xlab("Residuals")
```
...
Model 5 **is** Valid. Residuals are mostly normal, linearity checks out. ????????
...
Model 6 **is** Statisitcally Significal. Checking Validity
...
``` {r m6_check2}
# Residual Dist.
ggplot(data = m6, aes(x = .resid)) +geom_histogram(binwidth =1) + xlab("Residuals")
```
...
Model 6 **is** Valid. Residuals mostly normal, and linearity test shows zero slope. 
...
| Adult Homeless Pop. & Total Recovered Syringe Count (Logged) | Community District   | log(total_syringe_ests + 1) | log(avg_homeless_count + 1)           | 0.1096    | 0.1037             | 2.914e-05       |
| Adult Homeless Pop. & Total Housing Proj. Prev 5 Yrs. (Logged) | Borough              | log(avg_homeless_count + 1) | log(housingsum + 1)                   | 0.6862    | 0.6741             | 5.27e-08         |
| Adult Homeless Pop. & Avg. YOY Pct. Chg. Housing Proj. Prev 5 Yrs. (Logged) | Borough              | log(avg_homeless_count + 1) | log(avg_yoy_change + 1)               | 0.1922    | 0.1497             | 0.04681         |
| Adult Homeless Pop. & Total Recovered Syringe Count (Logged) | Borough              | log(total_syringes + 1)     | log(avg_homeless_count + 1)           | 0.2189    | 0.1889             | 0.01205         |



#### Community District Level Results
For the analysis at this Grographic level there was two models with a p-value indicating statistical significance. The first was the relationship between the Average Annual Adult Homeless and the total number of Completed Housing related construction projects. About 27% of the variance in the Homeless population can be explained by the Completed Construction Proejcts. The correlation was direct, which was unexpected. Essentially, the model showed that as the number of completed housing related construction projects for the previous 5 years increased, so did the homeless population. This was surprising because one would expect a negative sloped correlation, or that more construction projects completed the less homeless people. These results refute the Null hypothesis that there is no relationship between the variables, and confirms the alternative hypothesis of the independent variable, completed housing construction projects, impacting the homeless population. However, as mentioned, the result was opposite of what one may expect with a larger homeless popluation correlating with more completed housing projects in the preceeding 5 year window.

The second model that was statistically significant at the Community Distict level, was the direct relationship between the number of syringes collected in NYC Parks and from public disposal sites and the size of the adult homeless population. Roughly 10% of the variance in Syringe Collection counts can be explained by the size of the adult homeles population. These results reject the null hypothesis and confirm the alternative hypothesis. A reduction in the homless popuulation does reduce the number of syringes recovered.
...
Overall, the null hypotheses were refuted for both questions, with both alternative hypotheses being confirmed. The number of completed housing related construction projects in the preceeding 5 years for an area does in fact correlate to the size of homeless adult shelter population. Similarly, the size of the adult homeless popualtion does correlate with the number of syringes collected from parks and other public disposal units. 

The direct relationships between total construction projects completed and homeless population data may be explained by wealthier and gentrifying parts of the city consistently have the attention and resources of developers to create more market-rate and luxury housing via these construction project. The proliferation of such projects may exacerbate the issues surrounding afforabable housing. In my view, this is confirmed by the the indirect relationship between  the five year average YOY percent change in construction project and the homeless population. Larger YOY increases in completed construction projects may imply a sudden increase in completed construction jobs in areas typically neglected by high-end realestate developers and development general, thus having an indirect relationship with the homless population. A large average YOY increase in completed construction projects may also imply city planning aimed at helping areas in need of more housing, again helping explain the indirect relationship. 
...
Lastly, the adult homeless population data used in this analysis orginated at counts from the shelter system. This does not take into account the unhoused population that makes no use of the shelter system, so there may be a large blind spot not considered in this analysis. 
